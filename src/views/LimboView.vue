...
onMounted(() => {
  ...
  // ── API Reference Log ──────────────────────────────────────────────────
  console.log(
    '%c🚀 Web DiceBot Simulator — Limbo API Ready',
    'background:#f01d1d;color:#fff;font-weight:800;font-size:14px;padding:4px 12px;border-radius:6px'
  )
  console.log(
  ...
  console.table({
    'DiceSim.bet(amount, target)':  { description: 'Place a single Limbo bet. target = multiplier (e.g. 2.0)', returns: 'Promise<result>' },
